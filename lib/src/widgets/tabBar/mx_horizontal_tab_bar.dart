...
import 'package:mx_widget/src/widgets/tabBar/mx_tab.dart';
...
class MXHorizontalTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// and the length of the [MXHorizontalTabBarView.children] list.
  ...
  /// and the entire [MXHorizontalTabBar] is scrollable. Otherwise each tab gets an equal
  ...
  /// [MXHorizontalTabBar] in a [Padding] widget. When [isScrollable] is true, the scrollable itself is inset,
  ...
  /// An optional callback that's called when the [MXHorizontalTabBar] is tapped.
  ///
  /// The callback is applied to the index of the tab where the tap occurred.
  ///
  /// This callback has no effect on the default handling of taps. It's for
  /// applications that want to do a little extra work when a tab is tapped,
  /// even if the tap doesn't change the TabController's index. MXHorizontalTabBar [onTap]
  /// callbacks should not make changes to the TabController since that would
  /// interfere with the default tap handler.
  final ValueChanged<int>? onTap;

  /// How the [MXHorizontalTabBar]'s scroll view should respond to user input.
  ///
  ...
  /// Returns whether the [MXHorizontalTabBar] contains a tab with both text and icon.
  ///
  /// [MXHorizontalTabBar] uses this to give uniform padding to all tabs in cases where
  ...
  @override
  State<MXHorizontalTabBar> createState() => _MXHorizontalTabBarState();
}
...
class _MXHorizontalTabBarState extends State<MXHorizontalTabBar> {
  ...
  // If the MXHorizontalTabBar is rebuilt with a new tab controller, the caller should
  ...
  @override
  void didUpdateWidget(MXHorizontalTabBar oldWidget) {
  ...
  void _handleTabControllerAnimationTick() {
    ...
    if (!_controller!.indexIsChanging && widget.isScrollable) {
      // Sync the MXHorizontalTabBar's scroll position with the MXHorizontalTabBarView's PageView.
      _currentIndex = _controller!.index;
  ...
  BoxDecoration? _getContentDecorateInner(int index) {
    if (widget.outlineType == MXTabBarOutlineTypeEnum.capsule) {
      return BoxDecoration(
          color: index == _currentIndex
              ? MXTheme.of(context).brandColor1
              : MXTheme.of(context).infoColor1,
  ...
  BoxDecoration? _getContentDecorateOuter(int index) {
    ...
    } else if (widget.outlineType == MXTabBarOutlineTypeEnum.card) {
  ...
  Color? _getBackgroundColor(int index) {
    if (widget.outlineType == MXTabBarOutlineTypeEnum.card) {
      if (index == _currentIndex) {
        return MXTheme.of(context).infoColor1;
      }
    }
    return null;
  }
  ...
  Widget build(BuildContext context) {
    ...
    assert(() {
      if (_controller!.length != widget.tabs.length) {
        throw FlutterError(
          ...
          "number of tabs (${widget.tabs.length}) present in MXHorizontalTabBar's tabs property.",
        );
      }
      return true;
    }());
    ...
    if (_controller != null) {
      ...
      if (_controller!.indexIsChanging) {
        ...
        // The user is dragging the MXHorizontalTabBarView's PageView left or right.
    ...
    for (var index = 0; index < tabCount; index += 1) {
      wrappedTabs[index] = Opacity(
        opacity: widget.tabs[index].disabled ? 0.4 : 1,
        child: IgnorePointer(
          ignoring: widget.tabs[index].disabled,
          child: InkWell(
            ...
            child: Container(
              padding: widget.outlineType == MXTabBarOutlineTypeEnum.filled
    ...
    if (widget.isScrollable) {
      ...
      MXHorizontalTabBar = SingleChildScrollView(
        dragStartBehavior: widget.dragStartBehavior,
        scrollDirection: Axis.horizontal,
        controller: _scrollController,
        padding: widget.padding,
        physics: widget.physics,
        child: MXHorizontalTabBar,
      );
    } else if (widget.padding != null) {
      MXHorizontalTabBar = Padding(
        padding: widget.padding!,
        child: MXHorizontalTabBar,
      );
    }

    return MXHorizontalTabBar;
  }
}
...
class _TabBarScrollController extends ScrollController {
  ...
  final _MXHorizontalTabBarState tabBar;
...
/// selected tab.
///
/// This widget is typically used in conjunction with a [MXHorizontalTabBar].
///
...
/// [children] list and the length of the [MXHorizontalTabBar.tabs] list.
///
/// To see a sample implementation, visit the [TabController] documentation.
class MXHorizontalTabBarView extends StatefulWidget {
  /// Creates a page view with one child per tab.
  ///
  /// The length of [children] must be the same as the [controller]'s length.
  const MXHorizontalTabBarView({
    Key? key,
  ...
  /// One widget per tab.
  ///
  /// Its length must match the length of the [MXHorizontalTabBar.tabs]
  ...
  @override
  State<MXHorizontalTabBarView> createState() => _MXHorizontalTabBarViewState();
}

class _MXHorizontalTabBarViewState extends State<MXHorizontalTabBarView> {
  TabController? _controller;
  ...
  // If the MXHorizontalTabBarView is rebuilt with a new tab controller, the caller should
  // dispose the old one. In that case the old controller's animation will be
  // null and should not be accessed.
  bool get _controllerIsValid => _controller?.animation != null;
  ...
  @override
  void didUpdateWidget(MXHorizontalTabBarView oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.controller != oldWidget.controller) {
      _updateTabController();
      _currentIndex = _controller!.index;
  ...
  Widget build(BuildContext context) {
    assert(() {
      if (_controller!.length != widget.children.length) {
        throw FlutterError(
          "Controller's length property (${_controller!.length}) does not match the "
          "number of tabs (${widget.children.length}) present in MXHorizontalTabBar's tabs property.",
        );
      }
      return true;
    }());
...
class TabPageSelector extends StatelessWidget {
  ...
  Widget _buildTabIndicator(
    ...
    if (tabController.indexIsChanging) {
      ...
      // The selection's offset reflects how far the MXHorizontalTabBarView has / been dragged
