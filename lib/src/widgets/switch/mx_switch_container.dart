...
///
/// {@tool dartpad}
/// This example shows a toggleable [MXSwitchContainer]. When the thumb slides to
...
/// {@tool snippet}
///
/// This sample shows how to use a [MXSwitchContainer] in a [ListTile]. The
...
class MXSwitchContainer extends StatefulWidget {
  ...
  @override
  State<MXSwitchContainer> createState() => _MXSwitchContainerState();
...
class _MXSwitchContainerState extends State<MXSwitchContainer>
  ...
  @override
  void didUpdateWidget(MXSwitchContainer oldWidget) {
...
// 因为存在子节点所以需要改成SingleChildRenderObjectWidget而非LeafRenderObjectWidget
class _CupertinoSwitchRenderObjectWidget extends SingleChildRenderObjectWidget {
...
double _kChildRadius = MXTheme.getThemeConfig().radiusLarge - 4;

double _kChildMargin = (_kSwitchHeight - _kChildRadius * 2) / 2;
...
class _RenderCupertinoSwitch extends RenderConstrainedBox {
  ...
  final _MXSwitchContainerState _state;
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final double thumbLeft = lerpDouble(
      trackRect.left + _kTrackInnerStart - _kChildRadius,
      trackRect.left + _kTrackInnerEnd - _kChildRadius - currentThumbExtension,
      visualPosition,
    )!;
    final double thumbRight = lerpDouble(
      ...
      trackRect.left + _kTrackInnerEnd + _kChildRadius,
      visualPosition,
    )!;
    final double thumbCenterY = offset.dy + size.height / 2.0;
    final Rect thumbBounds = Rect.fromLTRB(
      thumbLeft,
      thumbCenterY - _kChildRadius,
      thumbRight,
      thumbCenterY + _kChildRadius,
    );
    ...
    if (child != null) {
      context.paintChild(child!, Offset(thumbBounds.left + _kChildMargin, 0));
    }
  }
